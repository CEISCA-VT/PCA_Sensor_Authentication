...
REPORT_BASE_DIR = r"./reports"    # folder for outputs
...
def build_single_sweep_ids(device_files, reg_index, bit_length):
    ...
    n_comp=choose_pca_components(n_samples,n_features,bit_length)
    scaler=StandardScaler().fit(X)
    pca=PCA(n_components=n_comp)
    Xp=pca.fit_transform(scaler.transform(X))
    binary_ids={device_order[i]:binary_from_projection(Xp[i],n_comp) for i in range(len(device_order))}
    return device_order,binary_ids,{"scaler":scaler,"pca":pca,"ref_freq":REF_FREQ,"use_phase":USE_PHASE,"bit_length":n_comp}

def build_multi_sweep_ids(device_files, train_indices, bit_length):
    X,labels=[],[]
    for dev,files in sorted(device_files.items()):
    ...
    return binary_ids,{"scaler":scaler,"pca":pca,"ref_freq":REF_FREQ,"use_phase":USE_PHASE,"bit_length":n_comp}
...
def run_all_tests(folder, bit_length, report_dir):
    os.makedirs(report_dir, exist_ok=True)
    device_files=collect_device_files(folder)
    print("Devices:",list(device_files.keys()))

    # Single
    order,bin_ids,model=build_single_sweep_ids(device_files,PREFERRED_REG_INDEX_SINGLE,bit_length)
    ...
    # Multi
    bin_ids_m,model_m=build_multi_sweep_ids(device_files,PREFERRED_MULTI_TRAIN_INDICES,bit_length)
...
if __name__=="__main__":
    for bit_length in ID_BIT_LENGTHS:
        run_all_tests(
            DEVICE_FOLDER,
            bit_length,
            os.path.join(REPORT_BASE_DIR, f"01_{bit_length}reports"),
        )
