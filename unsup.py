"""Unsupervised reconstruction-autoencoder authentication baseline.

The model is trained only to reconstruct enrollment sweeps, with code-balance,
decorrelation, and quantization regularizers. It uses no device labels in its
loss. Authentication sweeps are strictly held out, and per-bit thresholds are
the enrollment-latent medians rather than an arbitrary zero threshold.
...
REPORT_DIR = r"./01_unsupervised_autoencoder_reports_revised"
ID_BIT_LENGTHS = [128]
...
def fit_model(panel, training_indices, bit_length, seed):
    ...
    for _ in range(EPOCHS):
        ...
        total.backward()
        optimizer.step()
        last_losses = [total, reconstruction, balance, decorrelation, quantization]
    fit_seconds = perf_counter() - started

    model.eval()
    with torch.no_grad():
        _, enrollment_latents = model(inputs)
        enrollment_latents = enrollment_latents.cpu().numpy()
...
def run_scenario(scenario, panel, bits, threshold, seed):
    ...
    try:
        for fold_name, training_indices, test_index in folds:
            ...
            for device in devices:
                ...
                queries.append({
                    "Method": "Unsupervised autoencoder", "Scenario": scenario,
            ...
            names = ["Final_Total_Loss", "Final_Reconstruction_Loss",
                     "Final_Balance_Loss", "Final_Decorrelation_Loss",
    ...
    common.plot_hamming(
        intra, inter, f"Unsupervised autoencoder: {scenario} ({bits}-bit)",
...
def main():
    ...
    for seed in RANDOM_SEEDS:
        for bits in ID_BIT_LENGTHS:
            for scenario in ("Single-sweep", "Multi-sweep"):
                print(f"Running unsupervised AE: {scenario}, {bits}-bit, seed={seed}")
    ...
    pd.DataFrame([{
        "Method": "unsupervised reconstruction autoencoder", "Epochs": EPOCHS,
