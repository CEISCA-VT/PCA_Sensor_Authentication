...
provide 64 or 128 independent bits. The legacy script repeated an 8-bit PQM
pattern to reach those nominal lengths, which changes the reported Hamming
distance without adding information. This version reports the natural 12-,
18-, and 24-bit capacities (4, 6, or 8 bits per scalar) and normalized Hamming
distances so they can be compared without pretending that repeated bits are
new entropy.
...
# Natural code lengths are 3 * BITS_PER_FEATURE. Eight bits per scalar is the
# maximum evaluated resolution because 256 bins are already close to the cohort
# size and higher nominal precision would not establish higher physical entropy.
BITS_PER_FEATURE_VALUES = [4, 6, 8]

# Fixed before held-out evaluation. This is the 18.75% rule used for the legacy
# handcrafted baselines, rounded down to an integer Hamming distance.
...
def load_raw_impedance(path):
    ...
    for kwargs in ({"skiprows": 32}, {"skiprows": 33}, {"skiprows": 1}, {}):
        try:
            ...
            frequency, impedance = _extract_candidate(frame)
            if len(frequency) >= 2:
...
def extract_features(path):
    ...
    left_frequency = (
        REFERENCE_FREQUENCIES_HZ[left[-1]]
        if len(left)
        else REFERENCE_FREQUENCIES_HZ[0]
    )
    right_frequency = (
...
def code_diagnostics(identifiers):
    ...
    if active_bit_count >= 2:
        ...
        upper = np.abs(
            correlation[np.triu_indices(active_bit_count, k=1)]
        )
    ...
    probabilities = (
        np.asarray(list(counts.values()), dtype=float) / len(identifiers)
    )
    entropy = float(
    ...
    return {
        ...
        "Mean_Absolute_Bit_Bias": float(
            np.mean(np.abs(bit_fraction - 0.5))
        ),
...
def bootstrap_device_mean(query_frame, column):
    ...
    draws = rng.choice(
        values, size=(BOOTSTRAP_REPEATS, len(values)), replace=True
    ).mean(axis=1)
    ...
    try:
        for enrollment_index, authentication_index in folds:
            ...
            authentication_features = np.vstack(
                [
                    extract_features(panel[device][authentication_index])
            ...
            if method == "Direct-Scalar":
                ...
                build = lambda values: direct_identifier(
                    values, minimums, maximums, bits_per_feature
                )
            elif method == "PQM":
                boundaries = pqm_parameters(enrollment_features, bits_per_feature)
                build = lambda values: pqm_identifier(
                    values, boundaries, bits_per_feature
                )
            else:
            ...
            for position, device in enumerate(devices):
                ...
                unique_correct = (
                    len(nearest_positions) == 1 and nearest_positions[0] == position
                )
                ...
                query_rows.append(
                    {
                        ...
                        "Mean_Impostor_Hamming_Normalized": float(
                            impostor.mean() / bit_length
                        ),
            ...
            fold_rows.append(
                {
                    ...
                    **error_statistics(
                        fold_intra, fold_inter, bit_length, fixed_threshold
                    ),
                    **diagnostics,
                }
            )
            for device, identifier in identifiers.items():
                identifier_rows.append(
                    {"Method": method, "ID_Bits": bit_length, "Fold": fold_name,
